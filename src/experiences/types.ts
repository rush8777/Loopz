...
export interface RuntimeGuideDefinition { steps: GuideStep[]; design: ExperienceDesign; behavior?: { layer?: ExperienceLayer } }
...
export interface RuntimeChecklistDefinition { title: string; description?: string; items: ChecklistItem[]; behavior: { position: "bottom-left" | "bottom-right"; order: "any" | "sequential"; dismissible: boolean; initialState: "expanded" | "collapsed"; showRemainingCount: boolean }; completionMessage: { title: string; description?: string; acknowledgeLabel: string }; builder: WidgetBuilderState }
export interface ChecklistProgress { stateId: string; collapsed: boolean; dismissed: boolean; complete: boolean; completionAcknowledged: boolean; completedItemIds: string[]; items: Array<{ id: string; state: "completed" | "available" | "locked" }> }
export type RuntimeDefinition = RuntimeWidgetDefinition | RuntimeGuideDefinition;
export type ExperienceLaunchContext = { source: "api" } | { source: "checklist"; sourceExperienceId: string; sourceItemId: string };
export interface DeliveredExperience { id: string; versionId: string; kind: ExperienceKind; widgetType: WidgetType | null; priority: number; interruptPolicy?: "queue" | "interrupt"; impressionId?: string; definition: RuntimeDefinition; launchContext?: ExperienceLaunchContext }
export interface DeliveredChecklist { id: string; versionId: string; kind: "checklist"; priority: number; definition: RuntimeChecklistDefinition; progress: ChecklistProgress }
...
export function isChecklistDefinition(value: RuntimeDefinition | RuntimeChecklistDefinition): value is RuntimeChecklistDefinition { return "items" in value; }
